    ...
    <div class="col-md-12">
        <div class="widget box">
            <div class="widget-header">
                <h4><i class="icon-reorder"></i>Select device to remove template</h4>
            </div>
...
<div class="row" align="center">
    <input type="submit" value="Detach Now" id="detachnow" class="btn btn-primary" style="width: 150px;">
</div>
...
<script>
     $(document).on('click', '#detachnow', function(event) {
        ...
        if (selectedDevices.length==0){
            alert('Please select the device you want to detach this template');
        }else{
            ////
            $('#modal').iziModal('open');
            $('#modal').iziModal('setZindex', 99999999);
            message.innerText = 'Please be patient while templates are doing de-attach. Do not close this window.';
            jQuery.ajax({ 
                 headers: {
                 },
                 url: "{{ url('/admin/ciscosdwan/templates/detachcheck') }}",
                 method: 'post',
                 data: {
                    _token: '{{ csrf_token() }}',
                 ...
                 success: function(result,response){
                    ...
                    //console.log(result);
                    if (result==-1){
                        message.innerText = 'Please select the device you want to detach this template';
                    }else{
